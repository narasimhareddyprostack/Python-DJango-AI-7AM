...
@app.get("/users")
def get_users():
    return {"message":"Fetching users Successfully","users":users}
 """
...
'''
usage: fetch all users their age more than 25
API URL:http://127.0.0.1:8000/users?age=25&weight=50
API URL:http://127.0.0.1:8000/users?age=25&gender=male
Method:GET
Required Fields:None
Access Type:Public
'''
@app.get("/users")
def get_users_by_filter(age:Optional[int]=None,gender:Optional[str]=None):
    print(age)
    print(gender)
    new_users=users 
    """ if age:
        new_users=[user for user in users if user['age']>=age]
        return new_users """
    if age and gender:
        print('Test Case 123')
        new_users=[]
